...
## Auditoria tripla e mapa conectado — 25/08/2026

Foi realizada uma revisão em três passagens. Na primeira, foram conferidas as áreas e as coordenadas do mapa. Os fundos ligam ao corredor, o corredor à cidade, a cidade à estrada no eixo x -6 a 6 e a estrada à fazenda. Foram acrescentados marcos visuais no mundo (`CIDADE · LOTES`, `ESTRADA · FAZENDA` e `FAZENDA · CANTEIROS`), objetivo contextual no HUD e marcadores correspondentes no radar. A regressão `MAPA_CONEXOES_OK` confirma dez lotes dentro da cidade e o caminho contínuo entre as áreas.

Na segunda passagem, o multiplayer foi comparado com práticas de server authority, client prediction e reconciliation. O cliente remoto agora copia `loteIndex` ao ser criado e também em snapshots posteriores, corrige o rótulo para `Lote N` e nunca mais exibe `cliente do lote NaN`. Inputs de movimento passaram a carregar sequência; o servidor devolve a sequência nas correções; o cliente ignora correções antigas e não aplica pequenos ajustes de jitter que pareciam congelar o personagem. A validação de velocidade e colisão continua no servidor.
