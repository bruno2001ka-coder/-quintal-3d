...
const donoParaLote = new Map(); // playerId -> loteIndex
...
function tickPlanta(pl, dtSeg, clockMin) {
  ...
  pl.estagio = pl.prog >= 100 ? 3 : pl.prog >= 62 ? 2 : pl.prog >= 24 ? 1 : 0;
}
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.t === 'pos') {
      ...
      const q = Math.max(2, Math.round((1.3 + pl.s.t.rendimento / 100 * 2.6) * pl.saude * 7 * autoPen));
      lote.plots[pi] = null;
